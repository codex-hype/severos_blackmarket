    ...
    <main class="admin-dashboard">
        <section class="admin-hero">
            <div class="admin-hero-copy">
                ...
                <h1>Payments</h1>
                <p class="admin-lead">Review recent marketplace transactions, update statuses, and remove invalid entries from the ledger.</p>
            </div>
                                        ...
                                        <form method="post" class="inline-form">
                                            ...
                                            <select name="status" style="min-width: 120px;">
                                                <?php foreach (['Pending', 'Completed', 'Cancelled'] as $statusOption): ?>
                                                    <option value="<?php echo $statusOption; ?>" <?php echo $transaction['status'] === $statusOption ? 'selected' : ''; ?>><?php echo $statusOption; ?></option>
                                                <?php endforeach; ?>
                                            </select>
                                            <select name="method" style="min-width: 150px; margin-left:8px;">
                                                <?php foreach (['Debit Card','Credit Card','PayPal','Bank Transfer','Crypto'] as $methodOption): ?>
                                                    <option value="<?php echo $methodOption; ?>" <?php echo ($transaction['payment_method'] ?? 'Debit Card') === $methodOption ? 'selected' : ''; ?>><?php echo $methodOption; ?></option>
                                                <?php endforeach; ?>
                                            </select>
                                            <button type="submit" class="btn-secondary">Save</button>
                                        </form>
                                        <form method="post" class="inline-form" onsubmit="return confirm('Delete this transaction?');">
